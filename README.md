...
```bash
npm run dev                 # PostgreSQL + migraciones + seed + backend + frontend
npm run dev:app             # backend + frontend, sin tocar la base
npm run dev:fresh           # reinicia solo PostgreSQL, conserva imágenes/PDF, migra, seed y levanta la app
...
npm run db:setup            # PostgreSQL + migraciones + seed
npm run db:reset            # reinicia solo el volumen PostgreSQL; conserva backend_storage
...
El directorio raíz `scripts/` contiene el orquestador de desarrollo y el reinicio seguro de PostgreSQL. El segundo elimina únicamente el volumen de base de datos y nunca el volumen persistente de imágenes/PDF.
...
En Docker, el backend espera a PostgreSQL, ejecuta migraciones, garantiza el `super_admin` configurado y carga catálogo/blog antes de iniciar la API. No existen cuentas de desarrollo hardcodeadas: la única cuenta inicial proviene de `BOOTSTRAP_SUPER_ADMIN_*` y solo se crea cuando todavía no existe.
