...
import { MAX_PRODUCT_IMAGE_SIZE_BYTES, validateUpdateProductBody } from "./products.validation.js";
...
export const normalizeProductMultipartBody: RequestHandler = (req, _res, next) => {
    ...
    if ("categoryId" in req.body) {
        req.body.categoryId = parseCategoryId(req.body.categoryId);
    }
...
export function validateProductUpdateRequest(req: Request, _res: Response, next: NextFunction): void {
    try {
        ...
        if (Object.keys(body).length > 0) {
            validateUpdateProductBody(body);
        }
        next();
    }
    catch (error) {
