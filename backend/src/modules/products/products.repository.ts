...
export async function update(client: PoolClient, id: number, input: UpdateProductInput): Promise<Product | null> {
    const fields: string[] = [];
...
export async function hasCurrentImage(client: PoolClient, productId: number): Promise<boolean> {
    const result = await client.query(`SELECT 1 FROM stored_files WHERE ${currentImagePredicate} LIMIT 1`, [productId]);
    return Boolean(result.rowCount);
}
export async function replaceCurrentImage(client: PoolClient, input: SaveProductImageInput): Promise<void> {
