...
function assertSlug(
    ...
    if (slug.length > 120) {
        validationError(
            "slug no puede superar los 120 caracteres.",
        );
    }

    if (!SLUG_PATTERN.test(slug)) {
        validationError(
            "slug solo puede contener letras, números y guiones simples.",
        );
    }
}
...
function assertDescription(
    ...
    if (
        ...
        validationError(
            "description debe ser texto o null.",
        );
    }
}
...
export function validateCreateCategoryBody(
    value: unknown,
): asserts value is CreateCategoryInput {
...
export function validateUpdateCategoryBody(
    value: unknown,
): asserts value is UpdateCategoryInput {
    if (!isPlainObject(value)) {
        validationError(
            "El cuerpo de la solicitud debe ser un objeto JSON.",
        );
