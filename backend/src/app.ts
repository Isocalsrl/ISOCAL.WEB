...
const bundledFrontend = resolve(__dirname, "public");
if (existsSync(bundledFrontend)) {
    app.use(express.static(bundledFrontend, { index: false }));
    app.use((req, res, next) => {
        if (req.method === "GET" && req.accepts("html")) {
            res.sendFile(resolve(bundledFrontend, "index.html"));
            return;
        }
        next();
    });
