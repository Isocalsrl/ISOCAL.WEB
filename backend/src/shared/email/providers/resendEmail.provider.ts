...
export class ResendEmailProvider implements EmailClient {
    ...
    async send(message: EmailMessage, options: EmailSendOptions = {}): Promise<EmailSendResult> {
        try {
            ...
            if (error) throw new EmailProviderError("resend", "RESEND_ERROR", error.message);
            if (!data?.id) throw new EmailProviderError("resend", "RESEND_EMPTY_RESPONSE", "Resend no devolvió un identificador de correo.");
